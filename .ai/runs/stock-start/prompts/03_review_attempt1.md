...
- performance: medium
- output_file: .ai/features/stock-start/03_review.md
- result_json_file: .ai/features/stock-start/03_review.result.json
- run_state: .ai/runs/stock-start/run.json
- generated_at: 2026-05-28T11:44:58
- defaults_mode: true
...
3. Write the human-readable stage output file exactly at `.ai/features/stock-start/03_review.md`.
4. Also write the machine-readable stage result JSON exactly at `.ai/features/stock-start/03_review.result.json`.
...
The harness reads `.ai/features/stock-start/03_review.result.json` first. Keep the `## 단계 결과` section in `.ai/features/stock-start/03_review.md` for humans, but write this JSON file for the harness.
...
8. 리뷰 결과를 `.ai/features/stock-start/03_review.md`에 작성한다.
...
리뷰 완료 후 `.ai/features/stock-start/03_review.md`에 아래 형식으로 작성한다.
