...
- performance: medium
- output_file: .ai/features/stock-start/05_verify.md
- result_json_file: .ai/features/stock-start/05_verify.result.json
- run_state: .ai/runs/stock-start/run.json
- generated_at: 2026-05-28T11:52:16
- defaults_mode: true
...
3. Write the human-readable stage output file exactly at `.ai/features/stock-start/05_verify.md`.
4. Also write the machine-readable stage result JSON exactly at `.ai/features/stock-start/05_verify.result.json`.
...
The harness reads `.ai/features/stock-start/05_verify.result.json` first. Keep the `## 단계 결과` section in `.ai/features/stock-start/05_verify.md` for humans, but write this JSON file for the harness.
...
## Current Git Hints
- current_head: 69360e082a974bf0788fe0b8222cf3779cf8000d
...
- 하네스 실행 결과는 `.ai/runs/stock-start/verification/latest.json`에 저장된다.
...
4. `.ai/features/stock-start/02_dev.md`를 읽고 기능 목표, 구현 의도, Git 정보를 파악한다.
5. `.ai/features/stock-start/03_review.md`를 읽고 리뷰 지적 사항을 파악한다.
6. `.ai/features/stock-start/04_fix.md`를 읽고 수용/거부/보류 판단과 실제 수정 내용을 파악한다.
7. 의사결정 검증을 수행한다.
8. 동작 검증을 수행한다. 직접 실행한 명령은 기록하되, 최종 자동화 판정은 하네스가 실행하는 검증 명령 결과에 위임한다.
9. 검증 결과를 `.ai/features/stock-start/05_verify.md`에 작성한다.
10. 검증 시작 시점의 현재 `HEAD`를 `verify_target_commit`으로 기록한다.
11. 모델 기준 판정이 `PASS`이면 `05_verify.md`와 검증 관련 변경을 하네스가 세 번째 커밋에 포함할 수 있게 워킹트리를 커밋 가능한 상태로 남긴다. 추천 커밋 메시지는 `stock-start[YYYYMMDD-hhmmss][05_verify]` 포맷을 사용한다.
...
검증 완료 후 `.ai/features/stock-start/05_verify.md`에 아래 형식으로 작성한다.
...
- verification_commit_message_suggestion: stock-start[YYYYMMDD-hhmmss][05_verify]
